...
public class ChessPiece : MonoBehaviour {
    ...
    public void Update() {
        ...
        if (pieceName.Equals("black king") && complete && transform.localRotation.z < 134f/180) {
            float dt = Time.deltaTime * 2;
            gameObject.transform.Rotate(Vector3.forward, dt * 93);
            Vector3 curr = transform.localPosition;
            transform.localPosition = new Vector3(curr.x - dt * 0.4f, curr.y + dt * 0.44f, curr.z);
        }
    }
    ...
    public void drop() {
        ...
        if (x >= 0 && x < 8 && z >= 0 && z < 8) {
            ...
            if (Mathf.Abs(transform.localPosition.x - x) < X_THRESHOLD
                ...
                //Debug.Log(pieceName + " " + x + " " + z);
                //make sure correct piece is moved to correct square
                if (pieceName.Equals("white knight") && x == 0 && z == 2) {
